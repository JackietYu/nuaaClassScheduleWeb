(function(global) {
    ...
    Timetables.prototype = {
        _init: function(option) {
            ...
            var palette;
            if (typeof style.palette === "boolean" && !style.palette) { palette = false } else { palette = style.palette ? (style.palette || []).concat(this.defaultPalette) : this.palette }
            ...
            Timetables.forEach(function(item, index) { if (item.length < TimetableTypeLength) { for (var i = 0; i < TimetableTypeLength - item.length; i++) { item.push("") } } });
            if (option.setNewOption) { this.el.removeChild(this.el.childNodes[0]) }
            ...
            if (merge) {
                Timetables.forEach(function(list, i) {
                    ...
                    list.forEach(function(item, index) {
                        if (!index) { return listMerge[i].push({ name: item, length: 1 }) }
                        if (item === (listMerge[i][index - 1] || {}).name && item) {
                            var sameIndex = (listMerge[i][index - 1] || {}).sameIndex;
                            if (sameIndex || sameIndex === 0) { listMerge[i][sameIndex].length++; return listMerge[i].push({ name: item, length: 0, sameIndex: sameIndex }) }
                            listMerge[i][index - 1].length++;
                            return listMerge[i].push({ name: item, length: 0, sameIndex: index - 1 })
                        } else { return listMerge[i].push({ name: item, length: 1 }) }
                    })
                })
            }
            ...
            timetable.forEach(function(values, index) {
                ...
                --(TimetableType[0] || [])[2];
                if (!((TimetableType[0] || [])[2])) { TimetableType.shift() }
                    ...
                    if (merge && listMerge[i][index].length > 1) {
                        ...
                        if (merge && listMerge[i][index].length === 0) {
                            ...
                            if (item && palette) {
                                ...
                                paletteIndex++;
                                if (paletteIndex > palette.length) { paletteIndex = 0 }
                            } else { if (item) { courseItem.className = "course-hasContent" } }
                            courseItem.innerText = item || ""
                        }
                    }
                    courseItem.onclick = function(e) {
                        var allList = document.querySelectorAll(".Courses-content ul li").forEach(function(v, i) { v.classList.remove("grid-active") });
            ...
            deepCopyTimetableType.forEach(function(item, index) {
                ...
                } else { handItem.innerText = item[0] || "" }
